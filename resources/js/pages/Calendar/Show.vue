...
<div class="lcars-row">
<div class="lcars-column">
<div class="lcars-element lcars-periwinkle-bg"></div></div>
</div>
<div class="lcars-row">
<div class="lcars-column">
<div class="lcars-elbow left-top lcars-blue-bg"></div>
</div>
<div class="lcars-bar horizontal lcars-dodger-blue-bg bottom"><input ref="calendarInput" type="text" name="calendarName" v-model="calendarName" :readonly="!isEditable" @click="handleClick" @blur="handleBlur" class="lcars-title"/></div>
<div class="lcars-bar lcars-blue-bg horizontal bottom right-end decorated"></div>
</div>
<div class="lcars-row">
<div class="lcars-elbow lcars-dodger-blue-bg left-bottom"></div>
<div class="lcars-bar lcars-blue-bg horizontal"><div class="lcars-title right">{{month}}</div></div>
...
const deleteCalendar = (calendarId: number) => {
	...
	router.delete(`/calendar/${calendarId}`, {
		onSuccess: () => {
			...
			const defaultCalendar = (page.props.auth.user as any).calendars?.find(
				(cal: any) => cal.is_default,
			)
			if (defaultCalendar) {
				router.get(
