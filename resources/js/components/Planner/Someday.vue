<template>
	...
	<div class="lcars-row">
	<div class="lcars-bar horizontal left-end decorated lcars-dodger-blue-bg "></div>
	<div class="lcars-bar horizontal lcars-mariner-bg">
        ...
        <div class="mx-5">
            <div class="grid grid-cols-3 gap-4 mb-4">
                ...
                <TaskInput v-for="i in (3 - (taskList.length % 3)) % 3" :key="'empty-' + i" :disabled="true" :due="null" :calendar="(page.props.calendar as Calendar).id" />
            </div>
            <div class="grid grid-cols-3 gap-4">
                <TaskInput :disabled="false" :due="null" :calendar="(page.props.calendar as Calendar).id" :class="'decorated lcars-blue-color'"/>
                <TaskInput v-for="i in (9-taskList.length)" :key="i" :disabled="true" :due="null" :calendar="(page.props.calendar as Calendar).id" />
            </div>
        </div>
    </div>
...
const taskList = computed(() => {
	return (
		(page.props.tasks as TypeTask[])?.filter(
			(task) => task.due_date === null || task.due_date === "",
		).map(task => ({
			...task,
			notes: task.notes || "",
			date: task.due_date || "",
			sub_tasks: Array.isArray(task.sub_tasks) ? task.sub_tasks.map(st => typeof st === 'string' ? st : st.name || '') : []
		})) || []
	)
})
